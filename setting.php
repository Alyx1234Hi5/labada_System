...
// Retrieve from database
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        $minimum_kilos = ''; // Default value or handle as needed
        $delivery_day = ''; // Default value or handle as needed
...
// Fetch the delivery and pick up prices from price table
        ...
        $delivery_charge = $rowDelivery['price'] ?? ''; // Default value if not found
        ...
        $pickup_charge = $rowPickup['price'] ?? ''; // Default value if not found

// Update settings when form is submitted
    if (isset($_POST['submit'])) {
        $minimum_kilos = $_POST['min_kilos'];
        $delivery_day = $_POST['delivery_date']; // Ensure this matches the input name
        ...
        $result = mysqli_query($conn, $sql);
        if ($result) {
            echo "<script>alert('Pickup charge updated successfully!');</script>";
        } else {
